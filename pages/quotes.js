...
export default function Quotes() {
  ...
  const quoteAPI = async () => {
    ...
    try {
      ...
      console.log(data);
      arrayOfData = data.data[0];
    } catch (error) {
      console.log(error);
    }

    try {
      setQuote(arrayOfData.quote);
      setAuthor(arrayOfData.author);
      setCategory(arrayOfData.category);
      ...
      <main className={styles.main}>
        <div className={styles.grid}>
          ...
          <div className={styles.quoteCont}>
            <QuoteCard quote={quote} author={author} category={category} />
          </div>
          <Button
            text="New Quote"
            handleClick={() => {
